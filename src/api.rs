...
async fn set_config(
    db_pool: &State<PgPool>,
    token: Result<JwtString>,
    config: Json<config::UserConfigDbEntries>,
) -> Result<(), response::Debug<anyhow::Error>> {
    let claims = token.map_err(response::Debug)?;
